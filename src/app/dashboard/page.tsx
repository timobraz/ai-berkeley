...
export default function Dashboard() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setMessage("");
    try {
      const resp = await fetch("https://flowing-magpie-sweet.ngrok-free.app/converse_report", {
        method: "POST",
        headers: {
      ...
      if (resp.body) {
        ...
        const processStream = async ({ done, value }: any) => {
          ...
          setMessages((prev) => [...prev.slice(0, prev.length - 1), prev[prev.length - 1] + text]);
  ...
  useEffect(() => {
    ...
    if (ticker) {
      axios
        ...
        .then((res) => {
          ...
          const vals = data.map((d: any) => {
            return d.greenhouse_gas_emissions_scope_1;
          });
          setCharData(vals);
          setNumData([
            data[length - 1].score_energy_used,
            data[length - 1].score_greenhouse_gas_emissions_scope_1,
            data[length - 1].score_greenhouse_gas_emissions_scope_2,
            data[length - 1].score_greenhouse_gas_emissions_scope_3,
            data[length - 1].score_water_withdrawn,
            data[length - 1].score_waste_generated,
          ]);
          setFactors([data[length - 1].description]);
      ...
      axios.get("https://flowing-magpie-sweet.ngrok-free.app/analyze_report").then((res) => {
        ...
        setRetreived(data);
        setCharData([
          data.greenhouse_gas_emissions.CO2[0] + data.greenhouse_gas_emissions.CH4[0] + data.greenhouse_gas_emissions.N2O[0],
          data.greenhouse_gas_emissions.CO2[1] + data.greenhouse_gas_emissions.CH4[1] + data.greenhouse_gas_emissions.N2O[1],
          data.greenhouse_gas_emissions.CO2[2] + data.greenhouse_gas_emissions.CH4[2] + data.greenhouse_gas_emissions.N2O[2],
          data.greenhouse_gas_emissions.CO2[3] + data.greenhouse_gas_emissions.CH4[3] + data.greenhouse_gas_emissions.N2O[3],
          data.greenhouse_gas_emissions.CO2[4] + data.greenhouse_gas_emissions.CH4[4] + data.greenhouse_gas_emissions.N2O[4],
        ]);
        setNumData([
          ...
          <div className="flex flex-col  flex-1 gap-8">
            ...
            <div className="flex gap-4 h-full">
              <div className=" bg-dots3 rounded-md  flex-1 h-full p-5 text-white w-full text-center font-bold gap-5">
                <p className="mb-10 mt-10 text-2xl">Biggest Climate-Contributing Factors</p>
          ...
          <div className="flex flex-1 h-[48rem] overflow-scroll sticky top-20 z-10">
            <div className=" bg-[#262626]  rounded-md px-8 py-4 flex-col flex justify-between h-max min-h-[48rem] w-full">
              <div className="text-white tracking-wide my-4 ">
                <Link href={`https://flowing-magpie-sweet.ngrok-free.app/get_file`} target="_blank" rel="noopener noreferrer">
                  <h4 className="my-2">Report</h4>
                </Link>
                <div className="text-xl text-white tracking-normal font-bold overflow-auto">
                  {messages.length > 0 ? (
                    <>
                      <h2 className="text-green-300">Analyzing report...</h2>
