...
export default function Dashboard() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        console.log("ANALYZE REPORT: ", res.data);
        const data = JSON.parse(res.data);
        setRetreived(data);
        setCharData([
          data.greenhouse_gas_emissions?.CO2[0] ||
            0 + data.greenhouse_gas_emissions?.CH4[0] ||
            0 + data.greenhouse_gas_emissions?.N2O[0] ||
            0,
          data.greenhouse_gas_emissions?.CO2[1] ||
            0 + data.greenhouse_gas_emissions?.CH4[1] ||
            0 + data.greenhouse_gas_emissions?.N2O[1] ||
            0,
          data.greenhouse_gas_emissions?.CO2[2] ||
            0 + data.greenhouse_gas_emissions?.CH4[2] ||
            0 + data.greenhouse_gas_emissions?.N2O[2] ||
            0,
          data.greenhouse_gas_emissions?.CO2[3] ||
            0 + data.greenhouse_gas_emissions?.CH4[3] ||
            0 + data.greenhouse_gas_emissions?.N2O[3] ||
            0,
          data.greenhouse_gas_emissions?.CO2[4] ||
            0 + data.greenhouse_gas_emissions?.CH4[4] ||
            0 + data.greenhouse_gas_emissions?.N2O[4] ||
            0,
        ]);
        setNumData([
          ...
          <div className="flex flex-1 h-[48rem] overflow-scroll sticky top-20 z-10">
            <div className=" bg-[#262626]  rounded-md px-8 py-4 flex-col flex justify-between h-max min-h-[48rem] w-full">
              <div className="text-white tracking-wide my-4 ">
                <Link
                  href={`https://flowing-magpie-sweet.ngrok-free.app/get_file`}
                  target="_blank"
